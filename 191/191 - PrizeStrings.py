...
def PrizeString(numL,numA,D):
	if D == 30:
		return 1
	if numL == 1:
		if numA == 2:
			if (numL,numA,D) not in cache:
				cache[(numL,numA,D)] = PrizeString(numL,0,D+1)
			return cache[(numL,numA,D)]
		else:
			if (numL,0,D+1) not in cache:
				cache[numL,0,D+1] = PrizeString(numL,0,D+1)
			if (numL,numA+1,D+1) not in cache:
				cache[numL,numA+1,D+1] = PrizeString(numL,numA+1,D+1)
		...
		if numA == 2:
			...
			if (numL+1,0,D+1) not in cache:
				cache[numL+1,0,D+1] = PrizeString(numL+1,0,D+1)
